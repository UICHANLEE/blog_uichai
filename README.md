...
- 수익형 콘텐츠에 맞춘 인터랙티브 커스텀 테마 `Odd Note`
...
- 초기 글 5편은 현재 구축 경험과 공식 자료 검증을 바탕으로 만든 시작 콘텐츠입니다. 본인의 말투와 추가 경험을 반영해 편집합니다.
...
테마 폴더만 WordPress 컨테이너에 읽기 전용으로 연결됩니다. 컨테이너 안에서 테마를 편집하지 말고 이 프로젝트의 `themes/odd-note`를 원본으로 관리하세요. `make backup`의 운영 설정 백업에도 이 원본이 포함됩니다.
...
- `configuration.tar.gz`: Compose, 운영 스크립트, Odd Note 테마 원본
